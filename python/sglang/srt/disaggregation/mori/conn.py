...
class MoriKVSender(CommonKVSender):
    ...
    def poll(self) -> KVPoll:
        ...
        if transfers_done:
            logger.info(
                f"MoriKVSender: All transfers finished for room {self.bootstrap_room}"
            )
        ...
        if now - self.init_time > 5.0 and (int(now - self.init_time) % 30 == 0):
            ...
            logger.info(
                f"MoriKVSender: Room {self.bootstrap_room} stuck transferring. {in_progress_count}/{len(self.transfer_statuses)} ops in progress."
            )
